...
- 1c++) [명료화] WCA 파견위원은 대회에서 WCA 파견위원의 역할을 하지 않아도 대회에 참석할 수 있다. "WCA 파견위원"은 대회의 지정된 WCA 파견위원에 의해 관련 책임을 맡도록 임명되지 않는 한 해당하지 않는다 ([규정 1c+](regulations:regulation:1c+) 참고).
...
- 1h+++) [추가] 선수는 예정된 시간과 다른 시간에 별도의 조에서 기록 측정을 할 수 있도록 요청할 수 있다. WCA 파견위원은 이러한 요청을 승인하기 전에 상황의 공정성과 실용성을 신중히 고려해야 하며, 이러한 시도가 [규정 9l](regulations:regulation:9l)을 준수하는지 반드시 보장해야 한다(즉, 반드시 모든 선수가 동일한 종목의 이전 라운드에서 시도를 마친 후에 시작해야 하며, 임의의 선수가 동일한 종목의 후속 라운드에서 시도를 시작하기 전에 마쳐야 한다). 이러한 편의를 요청하는 선수는 대회 전에 운영팀과 WCA 파견위원에게 연락해야 한다. 예외: 이 지침은 3x3x3 최소회전 종목에서는 적용되지 않는다([규정 E6](regulations:regulation:E6) 참고).
    - 1h1) 한 라운드의 스크램블러와 심판은 해당 라운드에서 그들의 모든 시도가 완료되기 전에는 자신이 속한 조의 선수에 대하여 스크램블러/심판 업무를 절대로 해서는 안 된다. 이에 대하여 다음의 예외가 적용된다:
        ...
        - 1h1a+) [명료화] 3x3x3 최소회전 시도에 참가하고 있는 심판은 자신의 시도를 완료하기 전까지는 시도에 대한 채점을 절대로 시작해서는 안 된다. 시도를 일찍 제출한 경우, 60분이 끝나기 전에 채점을 시작할 수 있다([규정 E2b+](regulations:regulation:E2b+) 참고). 자신의 시도에 대한 채점은 다른 심판이 해야 한다.
...
- 3d) 퍼즐에는 퍼즐의 색 배치를 정의하는 색상 부품이 반드시 있어야 하며, 색상 부품으로 다음 중 반드시 오직 하나만 가질 수 있다: 색상 스티커, 색상 타일, 색상 플라스틱 혹은 색칠된/인쇄된 색. 퍼즐의 모든 색상 부품은 반드시 비슷한 재질로 만들어져야 한다. 예외: 클락 종목의 경우, 퍼즐에는 색상 부품이 없다.
    - 3d1) 의학적으로 입증된 시각 장애가 있는 선수에게는 다음의 예외가 적용된다:
        ...
        - 3d1b+) [명료화] additional uniform colors 혹은 무늬는 모든 면에 있을 필요는 없으며, 뚜렷한 색이 없는 면에만 있으면 된다. 조각은 같은 유형의 조각과 현저히 구분되는 특징(예: 질감, 패턴의 불규칙성)이 절대로 있어서는 안 된다.
...
- 4a) 스크램블러는 퍼즐에 스크램블 시퀀스를 적용한다.
    ...
    - 4a1+) [권장] WCA 파견위원은 5x5x5 큐브와 메가밍크스에 대해 맞추어지지 않은 퍼즐에 스크램블 시퀀스를 적용할 수 있는지 여부를 결정할 때 더욱 강력한 재량권을 사용해야 한다.
...
- 4b++) [상기] WCA 파견위원이 대회 도중에 추가적인 스크램블 시퀀스를 생성할 경우, 해당 스크램블 시퀀스는 반드시 저장되어야 한다.
    ...
    - 4b2+) [명료화] 일반적으로, 모든 공식적인 스크램블 시퀀스는 대회 동안 비밀로 유지되어야 하며 대회 종료 후에 한 번에 공개되어야 한다. 특별한 경우/특수한 상황의 경우(예: 세계 신기록), 운영팀이 한 그룹의 종료 직후 특정 스크램블 시퀀스를 조기에 공개하고자 할 수 있다.
...
- 4g+) [권장] 퍼즐이 올바르게 스크램블 되었다는 것을 검증하고 난 후, 스크램블러는 다른 스크램블러에게 퍼즐을 넘겨 검증을 받도록 할 수 있다. 이 경우, 두 스크램블러 모두 기록지에 서명해야 한다.
    - 4g1) 다음 예외가 적용된다:
        ...
        - 4g1a+) [명료화] 5x5x5 블라인드 종목에서, 잘못된 퍼즐 상태는 반드시 수정되어야 한다.
        - 4g1a++) [권장] WCA 파견위원은 5x5x5 큐브의 잘못된 퍼즐 상태를 수정해야 하는지 여부를 결정할 때보다 강력한 재량권을 사용해야 한다.
...
- 7f) 기록 측정석:
    - 7f1) 정의:
        ...
        - 7f1b+) [명료화] 풀 사이즈 매트의 최소 크기는 가로 30cm, 세로 25cm이어야 한다.
    ...
    - 7f3) 운영팀은 실수로 버튼이 눌리지 않도록 하고(예: 버튼 주위에 오링 부착) 배터리를 더욱 견고히 고정하는(예: 배터리 칸을 패딩 처리) 등 스피드솔빙 타이머를 견고히 개조함으로써 흔히 일어날 수 있는 사건들을 방지해야 한다.
...
- 9b++) [추가] [규정 9b](regulations:regulation:9b)에 명시된 것 외의 종목도 대회 중에 개최될 수 있지만, 비공식적인 것으로 간주하므로 대회의 공식 결과에 포함되지 않는다.
...
- 9p) 한 종목에 복수의 라운드가 있는 경우:
    ...
    - 9p2+) [명료화] 결과는 해당하는 라운드의 순위를 결정하는 단일 기록 혹은 평균 기록을 가리킨다.
        ...
        - 9p2a+) [추가] 종목의 추가 라운드는 해당 종목의 결승 라운드가 시작된 후에는 절대로 추가되어서는 안 된다. 단, 기존 결승 라운드에서 입상한 모든 선수가 변경에 동의하는 경우는 예외이다.
        - 9p2a++) [상기] 일대일 라운드 이후에는 추가 라운드를 절대로 추가해서는 안 된다. 이는 해당 일대일 라운드가 더 이상 종목의 결승 라운드가 아니게 되기 때문이다([규정 9b7b](regulations:regulation:9b7b) 참고).
...
- 9u+) [추가] 복수의 물리적 장소에서 진행되는 대회(즉, "3x3x3 최소회전 동시 대회")의 경우, 대회가 종료되었다고 간주하려면 반드시 모든 필요 사항이 모든 위치에서 충족되어야 한다.
    ...
    - 9u2+) [권장] WCA 파견위원은 대회의 종료를 발표하여야 한다(예: 시상식 전).
        ...
        - 9u2b+) [추가] 대회 종료의 달력 일자는 대회가 개최된 장소의 현지 시각을 기준으로 결정된다.
...
- 11e+++++++) [권장] 어느 라운드에 대한 엑스트라 시도가 후속 라운드가 종료된 후에 부여되었고, 선수가 엑스트라 시도로 인해 다음 라운드에 진출하게 되었다면, WCA 파견위원 재량에 따라 선수는 후속 라운드에 참가해서는 안 된다.
    ...
    - 11e1+) [상기] 엑스트라 시도는 반드시 공식 스크램블 프로그램에 의해 생성된 수정되지 않은 스크램블 시퀀스를 사용하여 섞어야 한다 ([규정 4b](regulations:regulation:4b), [규정 4b+](regulations:regulation:4b+) 참고).
...
- 11i) 선수가 솔브를 마친 후에 퍼즐이 올바르게 스크램블 되지 않았다는 것이 발견되면 다음 규정이 적용된다:
    ...
    - 11i1+) [권장] 이러한 기준을 충족할 것으로 예상되는 선수가 받게 되는 스크램블에 대해서는, 스크램블 된 큐브가 선수에게 전달되기 전에 두 번째 스크램블러에 의해 검사되어야 한다 ([규정 4g+](regulations:regulation:4g+) 참고).
...
- A1) 스피드 솔빙 시도는 반드시 다음 절차를 준수해야 한다.
    ...
    - A1a+) [명료화] 두 기록 제한이 모두 적용되는 경우, 각 시도에 대한 기록 제한은 절대로 누적 기록 제한보다 커서는 안 된다.
        ...
        - A1a3+) [상기] 운영팀과 WCA 파견위원은 기록 제한이 선수의 전략(예: 컷오프 라운드에서 컷오프를 충족하는 희망을 품고 처음 두 시도를 서두르는 것)에 영향을 준다는 것과, 라운드가 시작된 후에 기록 제한을 변경하는 것은 몇몇 선수에게 부당한 불이익을 줄 수 있다는 것을 반드시 유념해야 한다.
    ...
    - A1b+) [추가] 선수가 현재 소요 시간을 확인하기 위해 스톱워치를 집어들 수는 있지만, 이때 절대로 스톱워치의 시간 측정을 시작, 중지, 일시 정지하거나 다른 상호 작용을 해서는 안 된다. 예외: 심판이 즉시 스톱워치를 멈출 수 없는 경우, 선수가 자신의 시도 끝에서 스톱워치를 멈출 수 있다([규정 A6a1](regulations:regulation:A6a1) 참고).
    - A1b++) [추가] 운영진은 경과 시간을 확인하기 위한 비공식적인 스톱워치(주 스톱워치와 함께 시작)를 선수에게 제공할 수 있다. 이 경우, 선수는 주 스톱워치를 만질 수 없다. 예외: [규정 2i+](regulations:regulation:2i+)에 서술된 예외 참고.
        - A1b1) 스피드솔빙 타이머 반드시 스톱워치와 같이 사용되어야 한다. 예외: 선수가 10분을 초과할 것으로 예상되는 미리보기가 없는 종목에서, 스피드솔빙 타이머는 부수적이다.
        - A1b2) 스피드솔빙 타이머의 시간을 사용할 수 있는 경우, 이 시간이 본래의 기록된 시간이다. 그렇지 않다면(예: 스피드솔빙 타이머의 오작동, 스피드솔빙 타이머의 10분 이상 실행, 스피드솔빙 타이머가 전혀 사용되지 않음), 스톱워치의 시간이 본래의 기록된 시간이다.
...
- A2) 스크램블링:
    - A2a) 선수가 라운드에 호출되면, 맞추어진 퍼즐을 스크램블러에게 제출한다.
        ...
        - A2a3+) [상기] 선수는 모든 시도를 완료한 후에도 WCA 파견위원의 허가 없이 자신이 속한 그룹의 스크램블 시퀀스를 볼 수 없다 ([규정 4b2+](regulations:regulation:4b2+) 참고).
...
- A3) 미리보기:
    ...
    - A3c) 선수는 미리보기 동안 퍼즐을 만질 수 있다.
        ...
        - A3c5+) [상기] / 회전을 적용할 수 없는 퍼즐 상태는 유효한 스크램블 상태가 아니므로([규정 4b3d](regulations:regulation:4b3d) 참고), [규정 11i1](regulations:regulation:11i1)이 적용되는 경우 엑스트라 시도가 반드시 제공되어야 한다.
...
- A4) 솔브의 시작:
    - A4b) 선수는 스피드솔빙 타이머의 돌출된 센서 표면을 본인의 손가락으로 접촉한다. 선수의 양 손바닥은 반드시 뚜렷하게 아래를 향해야 하며, 타이머의 면 중 선수와 가까운 쪽에 위치해야 한다. 페널티: 시간 페널티(+2초).
        - A4b1) 선수는 솔브를 시작하는 동안 절대로 퍼즐과 물리적 접촉이 있어서는 안 된다. 페널티: 시간 페널티(+2초).
    - A4d) 스피드솔빙 타이머가 사용된다면, 선수는 타이머가 시작 준비 상태가 될 때까지 본인의 손을 타이머 위에 두어야 한다. 타이머는 선수가 타이머에서 손을 떼면 시작된다. 선수는 타이머를 시작함으로써 솔브를 시작한다.
...
- A7) 결과 기록:
    ...
    - A7b) 심판은 기록지에 결과를 기록하고 결과 옆에 서명함으로써 결과가 정확하고, 완전하며, 형식이 적절하고, 명확하게 읽을 수 있음을 인정한다. 심판 서명은 대회 전체에서 일관성이 있어야 하며, 심판이 누구인지 식별할 수 있어야 한다.
        ...
        - A7b2) 심판은 스피드솔빙 타이머에 표시된 추가 소수점 자릿수를 작성할 수 있지만, 이는 공식 결과에 포함되지 않는다([규정 9f](regulations:regulation:9f) 참고). 예: 스피드솔빙 타이머가 시도에 대해 12.678을 표시하는 경우, 올바르게 형식화된 시간은 12.67이지만 심판이 원하면 12.678을 작성할 수 있다.
...
- B2) 시도의 시작:
    ...
    - B2b) 스피드솔빙 타이머가 사용되는 경우, 선수는 타이머의 돌출된 센서 표면을 본인의 손가락으로 접촉한다. 선수의 양 손바닥은 반드시 아래를 향해야 하며, 타이머의 면 중 선수와 가까운 쪽에 위치해야 한다. 페널티: 시간 페널티(+2초).
...
- E2) 3x3x3 최소회전의 절차:
    ...
    - E2c+) [명료화] 심판이 해답을 제출할 수 있는 표준 종이를 제공할 수 있을지라도, 선수는 다른 종이에 해답을 제출할 수 있다. (이때, 종이는 반드시 [규정 E3a](regulations:regulation:E3a)에 따라 심판으로부터 제공받은 종이여야 한다.)
        ...
        - E2c3+) [명료화] 만약 회전이 명확히 지워지거나 삭제되지 않고 해당 회전이 해답의 일부인지 확실하지 않은 경우, 심판은 이를 모호한 회전 시퀀스로 간주해야 한다([규정 E2c2](regulations:regulation:E2c2)와 [규정 E2c2+](regulations:regulation:E2c2+) 참고).
...
- I3) 경기 구조:
    - I3a) 각 경기는 여러 세트로 구성된다. 경기의 승자는 X세트를 먼저 이기는 선수이다(X는 1, 2, 또는 3이다).
        - I3a1) X는 동일한 단계 내의 모든 경기에서 반드시 동일해야 한다. 예외: 결승전에서는 3위 결정전보다 더 큰 X 값을 사용할 수 있다.
    ...
    - I3d) 두 선수 중 어느 누구도 더 좋은 결과를 기록하지 못한 경우, 해당 포인트에는 승자가 없다.
    - I3e) 7포인트가 진행된 후에도 세트의 승자가 결정되지 않으면, 더 많은 포인트를 가진 선수가 세트를 승리한다. 두 선수의 포인트 수가 같으면 그 세트 내에서의 최고 단일 결과를 가진 선수가 세트를 승리한다. 그마저도 같을 경우에는 경기 전체에서의 최고 단일 결과와 라운드 전체에서의 단일 결과를 차례로 고려한다. 필요 시 최종 동점자 결정에 선수의 시드를 사용하며, 더 높은 시드를 가진 선수가 승리한다.
- I4) 일대일 스피드 솔빙 절차:
    ...
    - I4c) 미리보기:
        ...
        - I4c5) 미리보기 시간이 14초를 경과하면, 아나운서는 “시작”이라고 외친다. 이 시점에서 선수는 솔브를 반드시 시작해야 한다([규정 A4d](regulations:regulation:A4d) 참고). 고의적인 솔빙 시작 지연에 대한 페널티: 심판 재량으로 해당 시도 실격(DNF).
...
- I5) 일대일 블라인드 솔빙 절차:
    ...
    - I5b) 시도의 시작:
        ...
        - I5b2) 두 선수가 준비되었음을 확인하거나 최소 15초가 경과하면, 아나운서는 “3, 2, 1, 시작”이라고 외친다. 이 시점에서 선수는 솔브를 반드시 시작해야 한다([규정 B2d](regulations:regulation:B2d) 참고). 고의적인 솔빙 시작 지연에 대한 페널티: 심판 재량으로 해당 시도 실격(DNF).
